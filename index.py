...
root = Tk()
root.title("KC Inventory System")
...
def Database():
    ...
    cursor.execute("SELECT * FROM `admin` WHERE `username` = 'sahil' AND `password` = 'sahil'")
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `admin` (username, password) VALUES('sahil', 'sahil')")
        conn.commit()

def Exit():
    result = tkMessageBox.askquestion('KC Inventory System', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("KC Inventory System/Account Login")
    width = 600
    height = 500
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
def Home():
    global Home
    Home = Tk()
    Home.title("KC Inventory System/Home")
    width = 1024
    height = 520
    ...
    Title.pack(pady=10)
    lbl_display = Label(Title, text="KC Inventory System", font=('arial', 45))
    lbl_display.pack()
...
def ShowAddNew():
    global addnewform
    addnewform = Toplevel()
    addnewform.title("KC Inventory System/Add new")
    width = 600
    height = 500
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def Delete():
    if not tree.selection():
       print("ERROR")
        ...
        result = tkMessageBox.askquestion('KC Inventory System', 'Are you sure you want to delete this record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
...
def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("KC Inventory System/View Product")
    width = 600
    height = 400
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def Logout():
    result = tkMessageBox.askquestion('KC Inventory System', 'Are you sure you want to logout?', icon="warning")
    if result == 'yes':
        admin_id = ""
        root.deiconify()
        Home.destroy()
